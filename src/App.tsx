  ...
  return (
    ...
    <div className="flex h-screen bg-[#212121] text-white overflow-hidden">
      ...
      <div className="flex-1 flex flex-col bg-[#212121] min-h-0">
        ...
        {/* Main Chat Area */}
        {/*arrage children in column and min-h prevents from overflowing  */}
        <div className="flex-1 flex flex-col bg-[#212121] min-h-0">
          {currentMessages.length === 0 ? (
            // if currentMessages is empty show the greeting message and input bar in center. items-center centers content vertically
            // justify-center centers content horizontally so input is centered
            <div className="flex-1 flex flex-col items-center justify-center p-6 bg-[#212121]">
              ...
              <div className="w-full max-w-3xl">
                {/* input bar chnages e.target.value based on user click to user input*/}
                <div className="relative">
                  <input
                    type="text"
                    value={input}
                  ...
                  />
                  {/* absolute positioned div to place the send button on the right side of the input field */}
                  <div className="absolute right-3 top-1/2 -translate-y-1/2 flex items-center gap-2">
                    {/* if input is not empty show send button otherwise show mic and wrench */}
                    {input.trim() ? (
                      <button
            ...
            <>
              {/* scrollable message container with inline style to avoid vertical overflow */}
              <div
                className="flex-1 overflow-y-auto bg-[#212121]"
                style={{ minHeight: 0 }}
              >
                {/* limits width for messages and loops through array and chooses left or eight based on if its from user */}
                <div className="max-w-4xl mx-auto px-6 py-6 space-y-6 bg-[#212121]">
                  {currentMessages.map((msg, idx) => (
                    ...
                    >
                      {/* Avatar only shows for assistant messages on the left prevents avatar from shrinking*/}
                      {msg.role === "assistant" && (
                      ...
                      {/* Message Content for user allows increasing to cover content but to 70% of container*/}
                      <div
                      ...
                      >
                        ...
                        >
                          {/* Render user messages as plain text and assistant messages with markdown support */}
                          {msg.role === "user" ? (
                            <div>{msg.content}</div>
                          ) : (
                            <ReactMarkdown
                      ...
                      {/* Avatar only shows for user messages on the right */}
                      {msg.role === "user" && (
                        <div className="flex-shrink-0">
                          <div className="w-8 h-8 bg-gradient-to-br from-purple-500 to-pink-500 rounded-full flex items-center justify-center text-white text-sm font-medium">
                            S
